...
**Duas fotos, uma por formato.** Um `<picture>` entrega a peça vertical `900×1600` abaixo de
900px e a horizontal `1600×900` acima. O navegador baixa só a que vai usar.

A vertical existe porque 16:9 não preenche tela de celular sem perder o assunto: com `cover`
enchendo a altura, cabem só ~27% da largura, e os carros e o touro saem do enquadramento.
Como a peça 9:16 já enquadra a cena inteira, o `object-position` no celular volta a `50% 50%`
e a foto sangra ao fundo — **não há faixa 16:9 nesta página**. A raiz continua usando a faixa,
porque lá só existe a foto horizontal.

A marcação segue dividida em `.hero__in` (marca, palavras, lockup) e `.hero__baixo` (copy,
botão, chamarizes). A divisão nasceu para a faixa 16:9 poder entrar entre as duas; com a peça
vertical a faixa saiu, mas a divisão ficou — ela não atrapalha e evita refazer tudo se a
faixa voltar.
